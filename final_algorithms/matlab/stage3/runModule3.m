...
% One row per dev image, graded only by the fold model that never saw it.
report.out_of_fold_predictions = table(oof.ids, oof.grades, data.cv_fold(oof.mask), oof.resolutions, ...
    oof.scores, applyCutpoints(oof.scores, rules.cutpoints), ...
    applyCalibrator(rules.calibrator, oof.scores), oof.scores >= rules.operating_point.threshold, ...
    'VariableNames', {'id_code', 'diagnosis', 'cv_fold', 'resolution', 'grade_score', ...
